...
class ReplyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ava.setImageWith((viewModel?.tweet.profileUrl)!)
        addToolBar(textField: tweetTxtBox)

        tweetTxtBox.becomeFirstResponder()
        tweetTxtBox.delegate = self

        tweetTxtBox.text = (viewModel?.tweet.user?.screenName)!
    ...
    func addToolBar(textField: UITextView){
        ...
        tweetButton = UIBarButtonItem(title: "Reply", style: UIBarButtonItemStyle.plain, target: self, action: #selector(ComposeViewController.tweetPressed))
    ...
    func tweetPressed(){
        viewModel?.reply(t: tweetTxtBox.text)
